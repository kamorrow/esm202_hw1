...
Elevated levels of nitrogen and phosphorus reduce dissolved oxygen levels, pushing conditions towards a more reducing state.  The redox state of a water body influences the availability and toxicity of elements.
...
comparison <- comp_1980 %>% 
  ...
  mutate(total_nitrogen = total_nitrogen * 10 ^ 9  / 86400, # ug_s
         ...
         tp_concentration_ug_L = total_phosphorus / mean_flow_L_s,
         tn_concentration_ug_L = total_nitrogen / mean_flow_L_s)

ggplot(data = comparison) +
  geom_line(aes(x = year, y = tp_concentration_ug_L, color = river))

ggplot(data = comparison) +
  geom_col(aes(x = year, y = tp_concentration_ug_L, fill = river), show.legend = FALSE) +
  facet_wrap(~river) +
  labs(x = "", y = "Total Phosphorus (ug/L)") +

ggplot(data = comparison) +
  geom_col(aes(x = year, y = tn_concentration_ug_L, fill = river), show.legend = FALSE) +
  facet_wrap(~river) +
